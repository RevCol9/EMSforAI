...
- **serial_number**：资产序列号，唯一索引 `idx_devices_serial` 保证唯一。
- **location**：位置/站点。
- **status**：运行状态（默认为 `active`）。
- **created_at**：资产创建时间。
- **last_service_date**：最近保养时间。
...
### 新增资产与运维扩展表
- **`locations`**：分级站点/区域，支持父子层级（`parent_id`）。
- **`vendors`**：供应商或维保单位主数据。
- **`device_warranty`**：设备保修信息，与设备一对一（唯一索引 `idx_warranty_device`）。
- **`device_status_history`**：设备状态变更轨迹，索引 `idx_status_device_changed` 按设备/时间查询。
- **`work_orders`**：工单主表，按设备与状态、计划时间建立索引。
- **`maintenance_tasks`**：工单下的维护任务，索引 `idx_tasks_device_due` 支持按到期时间筛查。
- **`telemetry`**：原始遥测时序数据，索引 `idx_telemetry_device_metric` 覆盖设备/指标/时间。
- **`anomalies`**：异常记录（规则/模型/人工），索引 `idx_anomalies_device_detected`。
- **`health_scores`**：健康度计算结果，索引 `idx_health_device_computed`。
- **`maintenance_recommendations`**：AI/规则/人工给出的运维建议，按设备与优先级、创建时间建索引。
- **`audit_events`**：审计日志，记录操作人与实体，索引 `idx_audit_entity`、`idx_audit_created` 便于查询。
