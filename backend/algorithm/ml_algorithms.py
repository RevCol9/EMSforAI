...
CSV_PATH = r'D:\PythonProject\pythonProject\MCP\EMSforAI\data\csv'
...
def load_data_from_csv(base=CSV_PATH) -> pd.DataFrame:
    return {
        "device_metric_definitions": pd.read_csv(f"{base}/device_metric_definitions.csv").assign(
            feature_snapshot=lambda df: df["feature_snapshot"].apply(lambda x: json.loads(x) if isinstance(x, str) else x)
        ),
...
if __name__ == "__main__":
    ...
    test1 = prepare_metric_series(data=data, metric_key="spindle_vibration")