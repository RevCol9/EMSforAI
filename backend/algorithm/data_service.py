...
def prepare_metric_series(
    ...
    return df_irf.sort_values("record_time")[["record_time", "value", "quality"]]

def prepare_rul_series(data, metric_key, device_id=None, window_days=30):
    df = data["device_metric_definitions"]
    if device_id is not None:
        df = df[df[""]]